...
PushCat:
 ...
 function getMobileUA() {
     ...
     // 随机数
     return ua[GM_getValue("mobileUA", Math.floor(Math.random() * ua.length))];
 }
 ...
 function handler() {
     return getRewardsInfo().then(async resp => {
         ...
         const pcAttributes = dashboard.userStatus.
             counters.pcSearch[0].attributes;
 
         if (parseInt(pcAttributes.progress) >= parseInt(pcAttributes.max)) {
             // 判断是否有手机
             if (dashboard.userStatus.counters.mobileSearch) {
                 const mobileSearch = dashboard.userStatus.
                     counters.mobileSearch[0].attributes;
             ...
             await pushSend("必应每日签到完成", "当前等级: " + dashboard.userStatus.levelInfo.activeLevel +
                 ...
                 "\n可用积分: " + dashboard.userStatus.availablePoints + " 今日积分: " + dashboard.userStatus.
                     counters.dailyPoint[0].pointProgress);
             return true;
         } else {
             // 进行一次搜索
             GM_xmlhttpRequest({
